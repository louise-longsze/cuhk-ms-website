...
export const ListTable: React.FC<Props> = ({
  ...
  const deleteBloodSugar = useCallback(async (id: string) => {
    try {
      setDeletingBloodSugar(id);
      await fetch(`${process.env.NEXT_PUBLIC_APP_API_URL}/bloodSugars/${id}`, {
        method: "DELETE",
      });
  ...
  const deleteBloodPressure = useCallback(async (id: string) => {
    try {
  ...
  return (
    <div>
      ...
      <div className="pb-12">
        <div className="col-span-2 py-2 text-xl">血糖紀錄</div>
        {bloodSugars?.map((bloodSugar, key) => (
          <div key={key} className="bg-white rounded-lg">
            <div className="flex justify-between bg-blue-600 text-white px-4 py-2 rounded-t-lg">
              <div>日期：{bloodSugar.datetime}</div>
              <Button
                className="text-white px-4 py-2"
                type="button"
                variant="destructive"
      ...
      <div className="pb-12">
        <div className="col-span-2 py-2 text-xl">血壓紀錄</div>
        {bloodPressures?.map((bloodPressure, key) => (
          <div key={key} className="bg-white rounded-lg">
            <div className="flex justify-between bg-blue-600 text-white px-4 py-2 rounded-t-lg">
              <div>日期：{bloodPressure.datetime}</div>
              <Button
                className="text-white px-4 py-2"
                type="button"
                variant="destructive"
